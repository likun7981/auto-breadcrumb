...
const replaceParams = (names: string, params) => {
...
export default (
  url: string,
  {
     ...
     * No param
     */
    staticRoutesMap: { [key: string]: string },
    /**
     * With param
     */
    dynamicRoutesMap: { [key: string]: string | ((Object) => string) }
  }
) => {
  let names = staticRoutesMap[url];
  if (!names) {
    Object.keys(dynamicRoutesMap).some(pathname => {
      const pathMatchResult = matchPath(url, { path: pathname, exact: true, strict: true });
      if (pathMatchResult) {
        const { params } = pathMatchResult;
        names = dynamicRoutesMap[pathname];
        if (Array.isArray(names)) {
          names = names.map(name => replaceParams(name, params));
